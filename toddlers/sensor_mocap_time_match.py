...
"""
MOCAP & SENSOR STEP 1: 
Takes in two directories: Sensor and Motion Capture directories and goes through each sensor row
and finds the mocap entry with the time closest to it.
There should be pairings for each of the sensor and motion capture datasets based on the name! Should include
the same name (ex: both files have the name "Teresa")
"""
import argparse
...
def find_angle_match(angle_dir: Path, base_stem: str) -> Path | None:
    ...
    Should probably fix this where mocap just needs to have the name and sensor as well in each of their directories.
    Another input would be the name."""
