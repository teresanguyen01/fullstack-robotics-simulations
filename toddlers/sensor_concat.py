...
"""
SENSOR STEP 1:
Takes in individual files with the same format (#_(same name)_*) and concatenates the files. 
Turns the first time stamp to 0 and builds upon that time stamp after concatenation.
Uploads the output data into a folder.
"""
...
def fix_rollover_within_segments(ms_vals: np.ndarray, seg_ids: np.ndarray) -> np.ndarray:
    ...
    Just to be safe. There is probably no rollover.
    """
    out = ms_vals.copy()
    day_ms = 86_400_000.0
...
def main():
    ...
    for name in sorted(groups.keys()):
        ...
        cols = ["time_ms"] + [col for col in cat.columns if col != "time_ms"]
        cat = cat.reindex(columns=cols)

        out_path = out_dir / f"{name}_sensor.csv"
